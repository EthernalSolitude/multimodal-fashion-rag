...
![CI](https://img.shields.io/badge/CI-passing-brightgreen) ![Python](https://img.shields.io/badge/python-3.12-blue) ![FastAPI](https://img.shields.io/badge/FastAPI-async-009688) ![Qdrant](https://img.shields.io/badge/Qdrant-hybrid-red) ![Docker](https://img.shields.io/badge/docker--compose-ready-2496ED)
...
- **23 автоматических теста** (~0.6 сек, без GPU и БД), **CI на GitHub Actions** проверяет код на каждом push
- **Один `docker-compose up --build`** поднимает всё: API, Qdrant, Prometheus, Grafana
...
pytest                          # 23 теста, ~0.6 сек
ruff check .                    # линтер
```

**Что покрыто:** guardrail (пропуск/отказ/кеш/ошибка), reformulate, обработка пустого результата от LLM, построение фильтров Qdrant, определение языка запроса, сворачивание score в итоговый dict, санитизация входных фильтров.

Тесты не поднимают настоящие модели и Qdrant — заменяют их заглушками. Поэтому весь прогон ~0.6 сек и не требует GPU, интернета или запущенной БД. Живую интеграцию проверяет `docker-compose up`.
...
- **LLM-judge в `eval_diverse.py` капризничает** на малых моделях (llama3.1-8b через Cerebras) — нужна модель помощнее или детерминированный judge.
